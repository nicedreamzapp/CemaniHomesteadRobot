...
void calculateTankSpeeds(long lx, long ly, int16_t& leftSpeed, int16_t& rightSpeed) {
  ...
  // Convert to RPM
  leftSpeed = (int16_t)(leftPower * MAX_SPEED_RPM);
  rightSpeed = (int16_t)(rightPower * MAX_SPEED_RPM);
}
...
void setup() {
  ...
  Serial.println("  Synchronized Dual-Axis Control V2.0");
  Serial.println("========================================");
...
void loop() {
  ...
  if (controllerConnected && !emergencyStop && motorsEnabled) {
    if (now - lastMotorUpdate >= MOTOR_UPDATE_INTERVAL) {
      ...
      // Calculate target speeds from joystick
